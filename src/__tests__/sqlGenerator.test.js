...
//       const result = generateCreateAndInsertStatements(data, fields, tableName, tableType, batchSize);
...
//       const result = generateInClausesFromPaste(jsonData, batchSize);
...
//       const result = generateFullInClause(chunkedDataPoints, notIn, attributeName);
//       const normalizedResult = normalizeWhitespace(result);
//       const normalizedExpectation = normalizeWhitespace('(\n\t"id" IN (\n\t\t1,\n\t\t2\n\t)\nOR "id" IN (\n\t\t3,\n\t\t4\n\t)\n)');
//       expect(normalizedResult).toContain(normalizedExpectation);
//     });
...
//       const result = breakIntoChunks(allDataPoints, chunkSize);
...
//       const result = generateCreateTableSQL(fields, tableName, tableType);
  
//       expect(result).toBe('CREATE TABLE "users" ("id" INT, "name" VARCHAR(255));');
//     });
...
//       const result = generateInsertStatements(data, fields, tableName, batchSize);
  
//       expect(result).toBe('\n\nINSERT INTO "users" VALUES\n\t(\'1\', \'John\'),\n\t(\'2\', \'Jane\');');
//     });
  
//     test('generateInsertIntoClause', () => {
//       const tableName = 'users';
  
//       const result = generateInsertIntoClause(tableName);
  
//       expect(result).toBe('\n\nINSERT INTO "users" VALUES');
//     });
  
//     test('generateInsertLine', () => {
//       const insertIntoClause = '\n\nINSERT INTO "users" VALUES';
//       const includedFieldIndexes = [0, 1];
//       const row = [1, 'John'];
//       const totalRows = 1;
//       const rowNumber = 0;
//       const batchSize = 1;
  
//       const result = generateInsertLine(insertIntoClause, includedFieldIndexes, row, totalRows, rowNumber, batchSize);
  
//       expect(result).toBe('\n\nINSERT INTO "users" VALUES\n\t(\'1\', \'John\');');
//     });
  
//     test('isEndOfStatement', () => {
//       expect(isEndOfStatement(5, 4, 2)).toBe(true);
//       expect(isEndOfStatement(5, 3, 2)).toBe(false);
//       expect(isEndOfStatement(5, 4, null)).toBe(true);
//     });
  
//     test('isFirstLineOfStatement', () => {
//       expect(isFirstLineOfStatement(0, 2)).toBe(true);
//       expect(isFirstLineOfStatement(2, 2)).toBe(true);
//       expect(isFirstLineOfStatement(1, 2)).toBe(false);
//     });
  
//     test('validBatchSize', () => {
//       expect(validBatchSize(2)).toBe(true);
//       expect(validBatchSize(-1)).toBe(false);
//       expect(validBatchSize(null)).toBe(false);
//     });
...
// __tests__/sqlGenerator.test.js
